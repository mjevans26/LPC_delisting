...
plot_ly()%>%
  add_trace(data = new2, x = ~ (NDVI_max), y = ~ NDVI_var, color = ~ Class, type = "scatter", mode = "markers", text = ~paste(feat, count), hoverinfo = text)%>%
add_trace(x = ~c(0.2, 0.8), y = ~c(0.15, 0), type = 'scatter', mode = 'lines')
...
hist(ndvi_raw$disp[ndvi_raw$Class == 2 & ndvi_raw$NDVI_count > 5 & ndvi_raw$feat %in% tier2], freq = FALSE, breaks = 50)
...
plot(density(rgamma(5000, fit1var$estimate[1], fit1var$estimate[2]) - rgamma(5000, fit22var$estimate[1], fit22var$estimate[2])))
```
